  ...
  try {
    ...
    fields.forEach((field_name) => {
      if (!req.body[field_name]?.trim()) {
        ...
        errors.push(
          `The ${field_name.replace(/\_/g, " ")} must be at least 8 characters.`
        );
      }
    });
...
exports.login = async (req, res) => {
  try {
    ...
    let errors = [];
    if (!email?.trim()) errors.push("The email field is required.");
    if (!password?.trim()) errors.push("The password field is required.");
